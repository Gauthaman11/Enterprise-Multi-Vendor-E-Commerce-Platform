...
export default function Coupons() {
  ...
  async function load() {
    try { const res = await getCoupons(); setCoupons(res.data || []); }
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setForm({ code: "", discountType: "PERCENTAGE", discountValue: "", minOrderAmount: "", expiryDate: "", usageLimit: "", description: "" });
      setShowForm(false);
      load();
    } catch (e) { alert(e.response?.data?.message || "Failed to create coupon"); }
  }

  async function handleToggle(id) { await toggleCoupon(id); load(); }
            ...
            <tbody className="divide-y divide-stone-100">
              {coupons.map((c) => (
                <tr key={c.id} className="hover:bg-stone-50/40">
                  ...
                  <td className="px-5 py-4 text-stone-600">{c.expiryDate || "Never"}</td>
